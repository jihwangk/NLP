...
class LdaTopicCounts:
    ...
    def report(self, vocab, handle, limit=25):
        ...
        for kk in self._normalizer:
            ...
            word = 0
            for ww, _ in self._topic_term[kk].most_common(limit):
...
class Sampler:
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        if old_topic != -1:
            ...
            # Unassign the word from a topic
        ...
        if new_topic != -1:
            ...
            # Assign the word to the new topic
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            ...
            # uniform probability.
            sample_probs[kk] = (self._alpha[kk] + self._doc_counts[doc_id][kk]) * self._topics.word_in_topic(kk, term)
...
if __name__ == "__main__":
    ...
    lda.report_topics(vocab, args.output, limit=50)
